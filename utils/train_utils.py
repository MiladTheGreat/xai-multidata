...
def save_checkpoint(model,model_name,dataset_name,optimizer,epoch,best_acc,folder="checkpoints"):
    os.makedirs(folder,exist_ok=True)
    path = os.path.join(folder,f"{model_name}_{dataset_name}.pth")
    torch.save(
        {'epoch':epoch,
...
def save_predictions(model,dataloader,device,save_path):
    ...
    with torch.no_grad():
        for images, labels, paths in tqdm(dataloader,desc=f"logging the {os.path.basename(save_path)}"):
            ...
            for img_path,gt,pred,conf in zip(paths,labels,preds,confidence):
                rows.append({
    ...
    try:    
        ...
        save_path = save_path[:-4]+'_1.csv'
        pd.DataFrame(rows).to_csv(save_path,index=False)
        print(f'csv file was open, new_name is {os.path.basename(save_path)}')


def epoch_logger(epoch,train_loss,train_acc,val_loss,val_acc,log_path,dataset_name):
    csv_path = os.path.join(log_path,f"epoch_logs_{dataset_name}_dataset.csv")
    new_row = [{'epoch':epoch,
...
def train_loop(model,model_name,trainloader,testloader,optimizer,loss_fn,device,dataset_name,cfg):
    ...
    for epoch in range(num_epochs):
        ...
        epoch_logger(epoch,train_loss,train_acc,test_loss,test_acc,log_path,dataset_name)
        print(f"epoch:{epoch+1}/{num_epochs}: train loss: {train_loss:.4f} , acuracy: {test_acc:.2f}%")

        if test_acc > best_acc:
            best_acc = test_acc
            save_checkpoint(model,model_name,dataset_name,optimizer,epoch,best_acc)
        else:
            patience_counter+=1
    ...
    save_predictions(model,trainloader,device,os.path.join(log_path,f"preds_val_{dataset_name}_dataset.csv"))
